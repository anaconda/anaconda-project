...
def http_post_async(url, body=None, host=None):
    return _http_fetch(HTTPRequest(url=url, method='POST', body=body), host=host)
...
def http_post(io_loop, url, body=None, host=None):
    return io_loop.run_sync(lambda: http_post_async(url, body, host))
