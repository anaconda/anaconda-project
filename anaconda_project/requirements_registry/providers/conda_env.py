...
def _remove_env_path(env_path, project_dir):
    ...
    shortname = os.path.basename(env_path)
    if not os.path.isdir(env_path):
        msg = 'Conda environment "%s" not present; nothing to delete' % shortname
    ...
    try:
        ...
        msg = 'Failed to remove conda environment "%s": %s' % (shortname, str(e))
