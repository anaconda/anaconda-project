...
def test_main_fails_to_redis(monkeypatch, capsys):
    ...
    from conda_kapsel.commands.prepare_with_mode import prepare_with_ui_mode_printing_errors as real_prepare
    ...
    monkeypatch.setattr('conda_kapsel.commands.prepare_with_mode.prepare_with_ui_mode_printing_errors',
...
def test_prepare_command_choose_environment(capsys, monkeypatch):
    ...
    def check_prepare_choose_environment(dirname):
        ...
        result = _parse_args_and_run_subcommand(['conda-kapsel', 'prepare', '--directory', dirname, '--env-spec=bar'])
        assert result == 0
...
"""}, check_prepare_choose_environment)
    ...
    out, err = capsys.readouterr()
    assert out == (
        "The project is ready to run commands.\n" + "Use `conda-kapsel list-commands` to see what's available.\n")
    assert err == ""
...
def test_ask_variables_interactively(monkeypatch):
    def check(dirname):
        ...
        res = _parse_args_and_run_subcommand(['conda-kapsel', 'prepare', '--directory', dirname])
        assert res == 0
